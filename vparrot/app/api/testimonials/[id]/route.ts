...
export const GET = async (request: NextRequest) => {
    ...
    try {
        ...
        if (!testimonial) {
            return new NextResponse(JSON.stringify({ message: 'Témoignage non trouvé' }), { status: 404 });
        }

        return new NextResponse(JSON.stringify(testimonial), { status: 200 });
    } catch (err) {
        return new NextResponse(JSON.stringify({ message: 'Error fetching testimonial' }), { status: 500 });
    } finally {
...
export const POST = async (request: NextRequest) => {
    try {
        ...
        return new NextResponse(JSON.stringify({ message: 'Error creating testimonial' }), { status: 500 });
    }
};
...
export const PUT = async (request: NextRequest) => {
  try {
    ...
    return new NextResponse(JSON.stringify({ message: 'Error updating testimonial' }), { status: 500 });
  }
};
...
export const DELETE = async (request: NextRequest) => {
    try {
        ...
        return new NextResponse(JSON.stringify({ message: 'Testimonial deleted successfully' }), { status: 200 });
    } catch (err) {
        return new NextResponse(JSON.stringify({ message: 'Error deleting testimonial' }), { status: 500 });
    } finally {
        await prisma.$disconnect();
    }
};
